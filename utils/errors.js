...
export const createAuthenticationError = (message = 'Authentication failed', extra = {}) => {
    const error = createBaseError(message, 401, extra);
    error.name = 'AuthenticationError';
    return error;
};

export const createAuthorizationError = (message = 'Not authorized to access this resource', extra = {}) => {
    const error = createBaseError(message, 403, extra);
    error.name = 'AuthorizationError';
    return error;
};

export const createNotFoundError = (message = 'Resource not found', extra = {}) => {
    const error = createBaseError(message, 404, extra);
    error.name = 'NotFoundError';
    return error;
};

export const createRateLimitError = (message = 'Too many requests', extra = {}) => {
    const error = createBaseError(message, 429, extra);
    error.name = 'RateLimitError';
    return error;
};

export const createDatabaseError = (message = 'Database operation failed', extra = {}) => {
...
export const createCacheError = (message = 'Cache operation failed', extra = {}) => {
    const error = createBaseError(message, 500, extra);
    error.name = 'CacheError';
    return error;
};

export const createFileError = (message = 'File operation failed', extra = {}) => {
    const error = createBaseError(message, 500, extra);
    error.name = 'FileError';
    return error;
};

export const createThirdPartyAPIError = (message = 'Third party API error', extra = {}) => {
    const error = createBaseError(message, 502, extra);
    error.name = 'ThirdPartyAPIError';
    return error;
};

export const createTimeoutError = (message = 'Operation timed out', extra = {}) => {
...
export const formatError = (err, includeStack = false) => ({
    ...
    ...(err.extra && Object.keys(err.extra).length > 0 ? { details: err.extra } : {})
});
